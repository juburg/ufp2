...
void loop() {
  if (!GpsBaseAvailable) {
    if (opMode == 0) {        // TX modes
      ...
      if (rf69.available()) {
        ...
        if (rf69.recv(recBuf, &len)) {
          ...
          if (len + rtcmBufIndex > BUFLEN) {
            rtcmBufIndex = 0;
#ifdef debug
            Serial.println("Invalid rtcm message length");
#endif
          }
          for (int i=0; i< len; i++) {
            rtcmBuf[rtcmBufIndex+i] = recBuf[i];
          }
          rtcmBufIndex += len;
          if (len < RH_RF69_MAX_MESSAGE_LEN) {
            // end of message block
            // forward received RTK data stream to GPS
            Serial1.write(rtcmBuf,rtcmBufIndex);
            rtcmBufIndex = 0;
          }
          handleLED(50);
        }
    ...
    if (millis() - gpsLastCharRed > 50) {
      // end of message block
      if (!gpsEndOfBlock) {
        // do only once per message block
        gpsEndOfBlock = true;
        state = 0;
        msgLength = msgBufIndex;
        msgBufIndex = 0;
        while (msgLength) {
          if ((int)msgLength - RH_RF69_MAX_MESSAGE_LEN >= 0) {
            rf69.send(&msgBuf[msgBufIndex], RH_RF69_MAX_MESSAGE_LEN);
            msgBufIndex += RH_RF69_MAX_MESSAGE_LEN;
            msgLength -= RH_RF69_MAX_MESSAGE_LEN;
          }
          else {
            if ((int)msgLength - RH_RF69_MAX_MESSAGE_LEN < 0) {
              rf69.send(&msgBuf[msgBufIndex], msgLength);
              msgBufIndex += msgLength;
              msgLength = 0;
            }
          }
          rf69.waitPacketSent();
    ...
    if (!gpsEndOfBlock) {
      ...
      while (msgBuf[msgBufIndex] > 0) {
        ...
        if (gpsSolutionFound) {
          ...
          msgCheckSum = 0;
          while ((msgBuf[msgBufIndex] != '*') && (msgBufIndex < BUFLEN-1)) {
