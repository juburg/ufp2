      ...
      if ((pulseDuration < 1350) && (pulseDuration > 995)) {
        ...
        if ((pulseDuration >= 1350) && (pulseDuration < 3000)) {
          ...
          if ((engineSignal == 1) && (onCount == 2)) {
            ...
            handleRace(3);  // got motor ON signal
          }
        }
        else {
          // ignore
...
//
//  handleRace Function called on various events:
//
//    - (0) START signal (ready to start race)
//    - (1) A signal
//    - (2) B signal
//    - (3) engine ON signal
//    - (4) intrusion check, called every cycle
//    - (5) update rount trip timer, called every ms
//
//  startSequence:
//    0: IDLE
...
//    2: engine ON
//
//  roundtripCounter: down counter, updated every ms
//
//  legCount: counts valid legs (a->B and B->A)
//
//  blockLegCount: blocks leg count due to intrusions or missed turns/signals
//

void handleRace(uint8_t m) {
  ...
  switch (m) {
    ...
    case 1:
      if (startSequence == 2) {
        intrusionMonitoring = !intrusionMonitoring; // toggle
        if (!intrusionMonitoring) {
          beep(BEEPTIME,1000,50);
          if (lastLegUpdate == 1) {
            lastLegUpdate = 0;
          ...
          if (engineSignal == 2) {
            beep(BEEPTIME,500,50);    // intrusion detected, low freq. beep
            handleLED(200);
          }
          else {
        ...
        beep(BEEPTIME,500,50);        // round not running, low freq. beep
        handleLED(200);   
      }
    break;
    ...
    // got engine ON signal (engineSignal==2)
    case 3:
      if ((startSequence == 1) && (!roundtripCounter)) {
        roundtripCounter = 200001;    // set counter/timer to 200s
        startSequence = 2;            // set to RUN
      }
    break;

    // check for intrusion (motor ON at wrong moment)
    case 4:
      if ((startSequence == 2)&&(intrusionMonitoring)&&(engineSignal==2)) {
        beepDebounce++;
        if (beepDebounce > 1) {
          beep(BEEPTIME,500,50);  // low freq. beep
          blockLegCount = true;;
          handleLED(200);
        }
      } else
    ...
    case 5:
      if (startSequence == 2) { 
        ...
        if (roundtripCounter > 1)
          ...
          if (roundtripCounter == 1) {
            roundtripCounter = 0;
            beep(RTEEPTIME, 2500, 50);  // high freq. beep, end of round!
